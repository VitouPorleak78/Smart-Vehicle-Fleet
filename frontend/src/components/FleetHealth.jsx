...
const IconThermostat = () => (
  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a3 3 0 01-1-2.236V5a3 3 0 116 0v5.764A3 3 0 0113 13v6a2 2 0 11-4 0z" />
  </svg>
);
...
export default function FleetHealth() {
  return (
    <div className="h-full overflow-hidden bg-[#f8f9ff] text-[#0b1c30]">
      <div className="h-full flex flex-col gap-6 p-6 xl:p-8 overflow-y-auto">
                    ...
                    <tr key={row.id} className={`group transition-colors ${row.status === 'Warning' ? 'bg-[#fff1f3] hover:bg-[#ffe3e8]' : 'hover:bg-[#f8f9ff]'}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-[#e5efff] flex items-center justify-center text-[#0266ff]">
                            <IconTruck />
                          </div>
                        ...
                        <div className="flex justify-center gap-6">
                          {row.stats.map((stat) => (
                            <div key={stat.label} className="flex flex-col items-center text-[10px] text-[#6b7280] gap-1">
                              <span className="text-current">{renderStatIcon(stat.icon)}</span>
        ...
        <section className="bg-white rounded-[28px] border border-[#c5c6cf] shadow-sm p-6">
          ...
          <div className="mt-8 bg-[#f8fafc] rounded-full h-3 overflow-hidden flex">
            <div className="h-full bg-rose-500" style={{ width: '25%' }} />
            <div className="h-full bg-[#0266ff]" style={{ width: '25%' }} />
...
function StatBlock({ label, value, status, statusClass }) {
  return (
    <div className="rounded-3xl border border-[#e5eeff] bg-[#f8f9ff] p-5">
      ...
      <p className={`mt-2 text-[10px] font-semibold uppercase tracking-[0.24em] inline-block px-2 py-1 rounded-md ${statusClass}`}>{status}</p>
    </div>
  );
}